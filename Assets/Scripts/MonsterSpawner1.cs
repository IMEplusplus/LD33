...
public class MonsterSpawner1 : MonoBehaviour {
    ...
    void OnTriggerEnter2D(Collider2D coll)
    {
        if(coll.tag == "Player")
        {
            ...
            if(coll.transform.position.y > transform.position.y)
                Instantiate(elfPrefab, transform.position + (Vector3.down) * 2, Quaternion.identity);

            else
                Instantiate(elfPrefab, transform.position + (Vector3.up) * 2, Quaternion.identity);
